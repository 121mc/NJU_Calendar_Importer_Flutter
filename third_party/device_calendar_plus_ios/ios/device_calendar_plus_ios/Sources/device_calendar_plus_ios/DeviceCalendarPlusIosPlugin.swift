...
public class DeviceCalendarPlusIosPlugin: NSObject, FlutterPlugin, EKEventViewDelegate, EKEventEditViewDelegate {
  ...
  private let calendarQueue = DispatchQueue(
    label: "to.bullet.device_calendar_plus_ios.calendar-io",
    qos: .userInitiated
  )
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    switch call.method {
    case "listCalendars", "listSources", "createCalendar", "updateCalendar",
         "deleteCalendar", "listEvents", "getEvent", "createEvent",
         "deleteEvent", "updateEvent":
      // Initialize lazy services on the platform thread before sharing them
      // with the serial worker. Swift lazy initialization is not thread-safe.
      _ = permissionService
      _ = calendarService
      _ = eventsService
      calendarQueue.async {
        self.dispatchMethodCall(call) { value in
          // Includes argument-validation failures, which can return directly
          // from handlers before reaching their usual main-queue completion.
          DispatchQueue.main.async { result(value) }
        }
      }
    default:
      // Permission requests, settings and EventKit UI stay on the main thread.
      dispatchMethodCall(call, result: result)
    }
  }

  private func dispatchMethodCall(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    switch call.method {
