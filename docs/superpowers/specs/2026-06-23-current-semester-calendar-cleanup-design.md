...
本设计覆盖三个小改动：同步卡片里的“当前学期删除”快捷操作、独立删除页面及入口移除，以及 WebView 登录页底部按钮栏移除。
...
- 删除 `lib/pages/generated_events_cleanup_page.dart`。
- 删除只覆盖该页面的 `test/generated_events_cleanup_page_test.dart`。
- 删除主页面对 `GeneratedEventsCleanupPage` 的 import 和构建入口。
- 删除或收敛 `CalendarSyncService` 中仅供独立删除页面使用的全量扫描/分组删除方法，保留当前学期删除所需能力。
...
主页面采用收口入口的方案：删除底部独立清理卡片，并删除背后的独立清理页面，让删除能力只出现在当前学期对应的“系统日历同步”卡片里。这样用户不会再点进全量扫描页面看到空结果或卡顿，也能明确当前操作只作用于正在处理的学期。

备选方案一：UI 调用现有 `syncEvents`，传入空事件 bundle 并开启覆盖删除。这个方案改动少，但语义不清晰，会把“删除”伪装成一次同步，容易引入 created/skipped 统计误导。

否决的备选方案二：保留独立清理页，并复用它的扫描分组后按当前学期删除。这个方案能覆盖多日历，但它不符合“跟覆盖删除一样”的要求，且会继续暴露全量扫描卡顿与空结果页面问题。

## 用户流程

用户先完成登录、拉取某个学期课表、加载手机日历并选择目标日历。

主页面不再展示底部独立“删除本软件生成的日程”卡片，也不再保留独立删除页面。用户若要删除导入日程，需要先拉取对应学期课表，让“系统日历同步”卡片出现，再使用该卡片底部的“删除当前学期导入日程”按钮。
...
主页面 `ListView` 不再追加 `_buildGeneratedEventsCleanupCard()`，并删除该 helper。`lib/pages/generated_events_cleanup_page.dart` 和对应页面级测试一起移除；如果 `CalendarSyncService.scanGeneratedEventGroups`、`deleteGeneratedEventGroup`、`deleteGeneratedEventGroups` 删除后没有其他调用方，也应同步移除，避免留下不可达的全量扫描能力。
...
- 主页面不再能导航到 `GeneratedEventsCleanupPage`。
...
- 原有“写入系统日历”覆盖删除行为不变。
- 删除独立 `GeneratedEventsCleanupPage` 后，不再保留该页面的专用 widget 测试。
...
- 项目中不再保留独立 `GeneratedEventsCleanupPage` 页面及其专用页面测试。
